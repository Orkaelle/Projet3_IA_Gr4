...
def creation_echantillon_mails(nbEchantillon, tailleEchantillon):
    ...
    for echantillon in range(1, nbEchantillon + 1):
        ...
        echantillonDir = os.path.join(
            dirname, 'echantillons', str(echantillon))
        os.makedirs(echantillonDir)

        # On pioche des mails aléatoires et on les copie dans le dossier
        # échantillon
        liste_rand = numpy.random.randint(
            1, mails_count + 1, tailleEchantillon)
        ...
        for repertoire, sousRepertoires, fichiers in os.walk(mailDir):
            for f in fichiers:
                if id_mail in liste_rand:
                    ...
                    os.rename(
                        os.path.join(
                            echantillonDir, f), os.path.join(
                            echantillonDir, str(id_mail)))
                id_mail += 1
        print(
        ...
        print(
            tailleEchantillon,
            "mails aléatoires ont été copiés dans le répertoire numéro",
            echantillon,
            "\n")

    print('OK -', nbEchantillon, "échantillon(s) de", tailleEchantillon,
          'mails créé(s) dans le(s) répertoire(s) cible(s).\n')


creation_echantillon_mails(3, 10)
